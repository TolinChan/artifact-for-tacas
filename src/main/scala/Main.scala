...
import synthesis.{Synthesizer, SynthesisConfig}
...
object Main extends App {
  ...
  else if (args(0) == "synthesizeV2") {
    val name = args(1)
    val K = if (args.length > 2) args(2).toInt else 3
    val maxPerTr = if (args.length > 3) args(3).toInt else 2
    val train = if (args.length > 4) args(4).toInt else 40
    val valid = if (args.length > 5) args(5).toInt else 40
    val maxLen = if (args.length > 6) args(6).toInt else 6
    val perProp = if (args.length > 7) args(7).toInt else 2
    val cfg = SynthesisConfig(K = K, maxPerTr = maxPerTr, trainTraces = train, valTraces = valid, maxTraceLen = maxLen, perProp = perProp)
    print("synthesizingV2 ")
    Synthesizer().synthesizeV2(name, cfg)
  }
