...
case class RawRequire(expr: String, msg: String) extends SolidityStatement {
  override def toString: String = s"require($expr,\"$msg\");"
}
...
object Statement {
  ...
  /**
    * Inject synthesized require conditions at the beginning of public transaction functions.
    * guardsByFunc: Map from function name (e.g., "refund") to list of Solidity boolean expressions.
    * If multiple guards exist for a function, they are OR'ed together.
    */
  def injectRequires(statement: Statement, guardsByFunc: Map[String, List[String]]): Statement = {
    // 归一化：去除外层成对括号与多余空白，便于识别平凡 true/false
    def normalizeTrivialCond(s0: String): String = {
      ...
      while (curr != prev) {
        prev = curr
        curr = stripOuterParensSafe(prev)
      }
      curr
    }
    def injectInto(s: Statement): Statement = s match {
      case DeclContract(name, inner) => DeclContract(name, injectInto(inner))
      case Seq(a, b) => Seq(injectInto(a), injectInto(b))
      case DeclFunction(name, params, returnType, body, metaData) => {
        // 最终过滤：仅当存在至少一个非平凡条件时注入；过滤 true/false（含括号包裹）和空串
        val rawConds: List[String] = guardsByFunc.getOrElse(name, List()).filter(_.nonEmpty)
        val normalized: List[String] = rawConds.map(normalizeTrivialCond)
        val conds: List[String] = normalized.filter(c => {
          val t = c.trim
          t.nonEmpty && t != "true" && t != "false"
        })
        if (metaData.isTransaction && conds.nonEmpty) {
          val condStr = conds.map(c => s"($c)").mkString(" || ")
          val req = RawRequire(condStr, "Synthesis guard failed")
          DeclFunction(name, params, returnType, _makeSeq(req, injectInto(body)), metaData)
        } else {
          DeclFunction(name, params, returnType, injectInto(body), metaData)
        }
      }
      case other => other
    }
    injectInto(statement)
  }
